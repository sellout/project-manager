# Used by the project-manager tool to present news to the user. The content of
# this file is considered internal and the exported fields may change without
# warning.
{
  newsJsonFile,
  ...
  layoutNews = entries: let
    mkTextEntry = entry: let
      flag =
        if newsIsRead entry
        then "read"
        else "unread";
