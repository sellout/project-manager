{
  ...
  inherit
    ...
    ;
in {
  options.examples = mkOption {type = types.attrsOf pm.types.gvariant;};

  config = {
    examples = with pm.gvariant;
      mkMerge [
        {bool = true;}
        {bool = true;}
    ...
    project.file."result.txt".text = let
      mkLine = n: v: "${n} = ${toString (pm.gvariant.mkValue v)}";
