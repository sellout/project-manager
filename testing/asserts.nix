...
in {
  ...
  config = lib.mkMerge [
    (lib.mkIf config.test.asserts.warnings.enable {
      project.file = {
        "asserts/warnings.actual".text =
          concatStringsSep ''

            --
          ''
          config.warnings;
        "asserts/warnings.expected".text =
          concatStringsSep ''
      ...
      nmt.script = ''
        assertFileContent \
          project-files/asserts/warnings.actual \
          "$TESTED/project-files/asserts/warnings.expected"
      '';
    })
    ...
    (lib.mkIf config.test.asserts.assertions.enable {
      project.file = {
        ...
        "asserts/assertions.expected".text =
          concatStringsSep ''
      ...
      nmt.script = ''
        assertFileContent \
          project-files/asserts/assertions.actual \
          "$TESTED/project-files/asserts/assertions.expected"
      '';
    })
  ];
}
