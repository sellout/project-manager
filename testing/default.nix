{
  ...
  modules =
    import ../modules/all-modules.nix {
      inherit lib pkgs;
      check = false;
      modules = lib.attrValues (import ../modules/modules.nix);
    }
    ++ [
      (
        {config, ...}: {
          _module.args = {
            ...
            pkgs = let
              ...
              stubbedPkgs =
                if overlays == []
                then scrubbedPkgs
                else builtins.traceVerbose "eval overlayed nixpkgs" (lib.foldr (o: p: p.extend o) scrubbedPkgs overlays);
            in
              lib.mkImageMediaOverride stubbedPkgs;
          };

          ## The minimal configuration needed by a project.
          project = {
            name = lib.mkDefault "test";
            ## By default, tests run with the current `stateVersion`. It can be
            ## set in tests to ensure the behavior is consistent with previous
            ## states.
            # stateVersion = lib.mkDefault releaseInfo.version.major;
          };
...
in
  import nmtSrc {
    ...
    testedAttrPath = ["project" "packages" "activation"];
    tests =
      builtins.foldl'
      (
        a: b:
          ...
          // (
            let
              imported = import (lib.path.append b "tests");
            in
              if lib.isFunction imported
              then imported {inherit lib pkgs;}
              else imported
          )
      )
      {}
